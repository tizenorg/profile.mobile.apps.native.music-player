...
static const char *g__now_playing_btn_part[]= {"control_previous", "control_play", "control_pause", "control_next", NULL};

static const char *
_mp_now_playing_screen_reader_find_button_part(const Evas_Object *edje_obj, const Evas_Object *part_obj)
{
	startfunc;
	...
	while (g__now_playing_btn_part[index]) {
		WARN_TRACE("g__now_playing_btn_part[index]:%s",g__now_playing_btn_part[index]);
		obj = (Evas_Object *)edje_object_part_object_get(edje_obj, g__now_playing_btn_part[index]);
		WARN_TRACE("obj:%p",obj);
		if (obj == part_obj) return g__now_playing_btn_part[index];
		++index;
	}
...
static void
_mp_now_playing_action_double_cb(void *data, Evas_Object *obj, Elm_Object_Item *item)
{
	startfunc;

	MpNpData_t *wd = (MpNpData_t *)data;
	MP_CHECK(wd);

	struct appdata *ad = mp_util_get_appdata();
	MP_CHECK(ad);

	const char *part = _mp_now_playing_screen_reader_find_button_part(elm_layout_edje_get(wd->now_playing), obj);
	MP_CHECK(part);
	...
	else if (!g_strcmp0(part, "control_play") || !g_strcmp0(part, "control_pause"))
	{
		if (wd->play_bt_clicked)
			wd->play_bt_clicked(wd->userdata);
	}
	else if (!g_strcmp0(part, "control_next"))
	{
...
static void
_mp_now_playing_double_tap_options_cb(void *data, Evas_Object *obj, Elm_Object_Item *item)
{
	startfunc;
	MpNpData_t *wd = (MpNpData_t *)data;
	MP_CHECK(wd);
...
static Eina_Bool
_mp_now_playing_view_progress_reader_cb(void *data,
                  Evas_Object *obj,
                  Elm_Access_Action_Info *action_info)
{
	...
	Evas_Object *progress_obj = obj;
	MP_CHECK_FALSE(progress_obj);

	DEBUG_TRACE("action_info->action_type:%d", action_info->action_type);

	if (ELM_ACCESS_ACTION_DOWN == action_info->action_type)
	{
                ad->music_pos-=10;
                _mp_now_playing_update_time(wd);
	}
	else if (ELM_ACCESS_ACTION_UP == action_info->action_type)
	{
                ad->music_pos+=10;
                _mp_now_playing_update_time(wd);
	}
	else if (ELM_ACCESS_ACTION_UNHIGHLIGHT == action_info->action_type)
	{
	        return EINA_FALSE;
	}
	else if (ELM_ACCESS_ACTION_HIGHLIGHT == action_info->action_type)
	{
		return EINA_FALSE;
	}

	double duration = mp_player_mgr_get_duration();
	double new_pos = ad->music_pos;
	int req_seek_pos = 0;
	...
	mp_player_mgr_set_position(req_seek_pos, NULL, NULL);

        return EINA_TRUE;
}

static void _access_highlighted_cb(void *data, Evas_Object * obj, void *event_info)
{
	startfunc;
	MP_CHECK(data);

	g_tts_located_part = (char *)data;
	endfunc;
}
...
static char* _mp_now_playing_title_text_tts_info_cb(void *data, Evas_Object *obj)
{
	MpNpData_t *wd =  data;
	MP_CHECK_NULL(wd);

	const char *playing_text = elm_object_text_get(wd->label);
	return g_strdup(playing_text);
}
...
static void _nowplaying_resize_cb(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
	MP_CHECK(obj);
	_set_layout(obj);
}
...
/*static void _mp_now_album_btn_clicked_cb(void *data, Evas_Object *obj, void *event_info)
{
...
/*static char * _mp_now_playing_thum_access_info_cb(void *data, Evas_Object *obj)
{
	startfunc;

	char *read_txt = GET_STR(MP_TTS_MUSIC_PLAYER_BUTTON);
	return g_strdup(read_txt);
}*/

static char * _mp_now_playing_progress_access_info_cb(void *data, Evas_Object *obj)
{
	startfunc;
	MpNpData_t *wd =  data;
	MP_CHECK_NULL(wd);
	...
	const char *playing_text = edje_object_part_text_get(_EDJ(wd->now_playing), "np_progress_text_playing");
	const char *total_text = edje_object_part_text_get(_EDJ(wd->now_playing), "np_progress_text_total");

	char *text = g_strconcat(GET_STR(MP_TTS_PROGRESS_PLAYING), " ", playing_text, GET_STR(MP_TTS_PROGRESS_TOTAL), " ", total_text, NULL);
	return text;
}

static char * _mp_now_playing_ff_rew_access_info_cb(void *data, Evas_Object *obj)
{
	startfunc;
...
static char * _mp_now_playing_play_pause_access_info_cb(void *data, Evas_Object *obj)
{
	startfunc;
...
/*static void _mp_now_playing_set_repeate_image(void *data, int repeate_state)
{
	MpNpData_t *wd = data;
        MP_CHECK(wd);
	Evas_Object *thumbnail = elm_image_add(wd->repeate_focus_btn);
	if (repeate_state == MP_PLST_REPEAT_NONE)
	{
		elm_image_file_set(thumbnail, IMAGE_EDJ_NAME, MP_MINI_REPEAT_ONCE_ICON);
	}
	else if (repeate_state == MP_PLST_REPEAT_ONE)
	{
		elm_image_file_set(thumbnail, IMAGE_EDJ_NAME, MP_MINI_REPEAT_1_ICON);
	}
	else if (repeate_state == MP_PLST_REPEAT_ALL)
	{
		elm_image_file_set(thumbnail, IMAGE_EDJ_NAME, MP_MINI_REPEAT_ALL_ICON);
	}
	elm_object_part_content_set(wd->repeate_focus_btn, "elm.swallow.content", thumbnail);
}

static void _mp_now_playing_set_shuffle_image(void *data, int shuffle_state)
{
	MpNpData_t *wd = data;
        MP_CHECK(wd);
	Evas_Object *thumbnail = elm_image_add(wd->shuffle_focus_btn);
	if (shuffle_state)
	{
		elm_image_file_set(thumbnail, IMAGE_EDJ_NAME, MP_MINI_SHUFFLE_ON_ICON);
	}
	else
	{
		elm_image_file_set(thumbnail, IMAGE_EDJ_NAME, MP_MINI_SHUFFLE_OFF_ICON);
	}

	elm_object_part_content_set(wd->shuffle_focus_btn, "elm.swallow.content", thumbnail);
}*/

/*static void _mp_now_playing_update_shuffle_repeat_btn(Evas_Object *obj)
{
	MP_CHECK(obj);
	MpNpData_t *wd = GET_WIDGET_DATA(obj);
	MP_CHECK(wd);
	...
	_mp_now_playing_set_repeate_image(wd, repeat_state);
	int shuffle_state = 0;
	mp_setting_get_shuffle_state(&shuffle_state);
	_mp_now_playing_set_shuffle_image(wd, shuffle_state);

}


static void _mp_now_playing_repeat_btn_clicked_cb(void *data, Evas_Object *obj, void *event_info)
{
	struct appdata *ad = mp_util_get_appdata();
	MP_CHECK(ad);
	...
	_mp_now_playing_set_repeate_image(data, repeat_state);
	mp_playlist_mgr_set_repeat(ad->playlist_mgr, repeat_state);
	if (ad->win_minicon)
		mp_minicontroller_update_shuffle_and_repeat_btn(ad);
}

static void _mp_now_playing_shuffle_btn_clicked_cb(void *data, Evas_Object *obj, void *event_info)
{
	struct appdata *ad = mp_util_get_appdata();
	MP_CHECK(ad);
	...
	_mp_now_playing_set_shuffle_image(data, shuffle_state);
...
static char * _mp_now_playing_shuffle_access_info_cb(void *data, Evas_Object *obj)
{
	startfunc;
	struct appdata *ad = mp_util_get_appdata();
	MP_CHECK_NULL(ad);
...
static char * _mp_now_playing_repeat_access_info_cb(void *data, Evas_Object *obj)
{
	startfunc;
	struct appdata *ad = mp_util_get_appdata();
	MP_CHECK_NULL(ad);
...
Evas_Object *mp_now_playing_create(Evas_Object *parent, MpNowplayingCb play_bt_clicked, MpNowplayingCb clicked, void *data)
{
	...
	if (playing_pannel)
	{
		/*if (landscape)
		{
			r = elm_layout_file_set(playing_pannel, PLAY_VIEW_EDJ_NAME, "landscape_mp_now_playing");
			evas_object_event_callback_add(playing_pannel, EVAS_CALLBACK_RESIZE, _nowplaying_resize_cb, NULL);
		}
		*/
