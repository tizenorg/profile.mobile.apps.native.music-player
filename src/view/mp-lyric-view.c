...
static Eina_List *_mp_lyric_view_get_line_list(void *data);
static Evas_Object *_mp_lyric_view_get_line(void *data, int index);
static void _mp_lyric_view_set_current_line(void *data, int index);
...
static void _mp_lyric_view_load_lyric(void *data);
static Evas_Object *_mp_lyric_view_create_layout(void *data);
static void _mp_lyric_view_init_data(void *data);
...
static Eina_List*
_mp_lyric_view_get_line_list(void *data)
{
	startfunc;
	MpPlayerView_t *playing_view = (MpPlayerView_t *)data;
	MP_CHECK_NULL(playing_view);
	MP_CHECK_NULL(playing_view->lyric_view);

	struct appdata *ad = mp_util_get_appdata();
	MP_CHECK_NULL(ad);

	Eina_List *list = NULL;
	mp_lyric_view_t *lyric_view = playing_view->lyric_view;
...
static Evas_Object*
_mp_lyric_view_get_line(void *data, int index)
{
	startfunc;
	...
	Eina_List *list = _mp_lyric_view_get_line_list(playing_view);
...
static void
_mp_lyric_view_set_current_line(void *data, int index)
{
	startfunc;
	...
	MP_CHECK(playing_view->lyric_view);
	...
	Evas_Object *line = _mp_lyric_view_get_line(playing_view, index);
	MP_CHECK(line);

	/* No change */
	if (playing_view->lyric_view->cur_line == line)
		return;

	/* Update current line */
	playing_view->lyric_view->prev_line = playing_view->lyric_view->cur_line;
	playing_view->lyric_view->cur_line = line;

	playing_view->lyric_view->prev_line_index = playing_view->lyric_view->cur_line_index;
	playing_view->lyric_view->cur_line_index = index;

	int x0, y0, w0, h0;
	Evas_Object *head = _mp_lyric_view_get_line(playing_view, 0);
	...
	int x, y, w, h;
	evas_object_geometry_get(playing_view->lyric_view->cur_line, &x, &y, &w, &h);
	DEBUG_TRACE("===Get current line====i =%d, x=%d, y=%d, w=%d, h=%d\n", index, x, y, w, h);

	int new_y = 0;
	if (playing_view->play_view_screen_mode == MP_SCREEN_MODE_LANDSCAPE) {
		int scale_h_ls = playing_view->lyric_view->win_w * MP_LRC_VIEW_H_SCALE_LS;
		int scale_w_ls = playing_view->lyric_view->win_h * MP_LRC_VIEW_W_SCALE_LS;
		int scale_head_h_ls = playing_view->lyric_view->win_w * MP_LRC_HEAD_H_SCALE_LS;
		...
		elm_scroller_region_bring_in(playing_view->lyric_view->scroller, x, new_y, scale_w_ls, scale_h_ls);
	} else {
		int scale_h = playing_view->lyric_view->win_h * MP_LRC_VIEW_H_SCALE;
		int scale_w = playing_view->lyric_view->win_w * MP_LRC_VIEW_W_SCALE;
		int scale_head_h = playing_view->lyric_view->win_h * MP_LRC_HEAD_H_SCALE;
		...
		elm_scroller_region_bring_in(playing_view->lyric_view->scroller, x, new_y, scale_w, scale_h);
	}

	_mp_lyric_view_set_line_color(ad, playing_view->lyric_view->cur_line, playing_view->lyric_view->cur_line_index, MP_LRC_LINE_COLOR_PLAYING);

	if (playing_view->lyric_view->prev_line != NULL) {
		_mp_lyric_view_set_line_color(ad, playing_view->lyric_view->prev_line, playing_view->lyric_view->prev_line_index, MP_LRC_LINE_COLOR_DEFAULT);
	}

	evas_object_show(playing_view->lyric_view->scroller);
...
_mp_lyric_view_drag_start(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
	...
	playing_view->lyric_view->b_drag = EINA_TRUE;
...
_mp_lyric_view_drag_stop(void *data, Evas *e, Evas_Object *obj, void *event_info)
{
	...
	playing_view->lyric_view->b_drag = EINA_FALSE;
...
static void
_mp_lyric_view_load_lyric(void *data)
{
	startfunc;
	...
	MP_CHECK(playing_view);
	MP_CHECK(playing_view->lyric_view);
	MP_CHECK(playing_view->lyric_view->scroller);
	MP_CHECK(playing_view->lyric_view->box);
	...
	elm_box_clear(playing_view->lyric_view->box);
	for (index = 0; index < count; index++) {
		mp_lrc_node_t *lrc_node = (mp_lrc_node_t *)eina_list_nth(list, index);
		MP_CHECK(lrc_node);
		/*DEBUG_TRACE("line%d=%s\n", index, lrc_node->lyric);*/
		Evas_Object *line = mp_common_load_edj(playing_view->player_view_layout, PLAY_VIEW_EDJ_NAME, "mp_lrc_line");
		edje_object_part_text_set(_EDJ(line), "txt", lrc_node->lyric);

		evas_object_show(line);
		evas_object_data_set(line, "time", &lrc_node->time);
		elm_box_pack_end(playing_view->lyric_view->box, line);
	}

	/* Add gap for head and tail */
	if (playing_view->play_view_screen_mode == MP_SCREEN_MODE_LANDSCAPE) {
		Evas_Object *head = mp_common_load_edj(playing_view->player_view_layout, PLAY_VIEW_EDJ_NAME, "mp_lrc_head_landscape");
		elm_box_pack_start(playing_view->lyric_view->box, head);
	} else {
		Evas_Object *head = mp_common_load_edj(playing_view->player_view_layout, PLAY_VIEW_EDJ_NAME, "mp_lrc_head");
		elm_box_pack_start(playing_view->lyric_view->box, head);
	}
...
static Evas_Object*
_mp_lyric_view_create_layout(void *data)
{
	startfunc;
	...
	/* Create layout */
	layout = mp_common_load_edj(playing_view->layout, PLAY_VIEW_EDJ_NAME, "mp_lrc_layout");
	evas_object_show(layout);
	playing_view->lyric_view->layout = layout;
	...
	playing_view->lyric_view->scroller = scroller;
	...
	playing_view->lyric_view->box = box;
	...
	/* Set event callback */
	/*evas_object_smart_callback_add(playing_view->lyric_view->scroller, "scroll", _mp_lyric_view_scroll_cb, ad);*/
	evas_object_event_callback_add(playing_view->lyric_view->scroller, EVAS_CALLBACK_MOUSE_DOWN, _mp_lyric_view_drag_start, playing_view);
	evas_object_event_callback_add(playing_view->lyric_view->scroller, EVAS_CALLBACK_MOUSE_MOVE, _mp_lyric_view_drag_move, playing_view);
	evas_object_event_callback_add(playing_view->lyric_view->scroller, EVAS_CALLBACK_MOUSE_UP, _mp_lyric_view_drag_stop, playing_view);
	endfunc;
...
static void
_mp_lyric_view_init_data(void *data)
{
	startfunc;
	...
	if (playing_view->lyric_view != NULL) {
		mp_lyric_view_destroy(playing_view);
	}

	/* Create view data */
	playing_view->lyric_view = (mp_lyric_view_t *)malloc(sizeof(mp_lyric_view_t));
	MP_CHECK(playing_view->lyric_view);
	memset(playing_view->lyric_view, 0, sizeof(mp_lyric_view_t));

	/* Create lyric layout */
	playing_view->lyric_view->layout = _mp_lyric_view_create_layout(playing_view);
	elm_object_part_content_set(playing_view->player_view_layout, "lrc", playing_view->lyric_view->layout);
	evas_object_event_callback_add(playing_view->lyric_view->layout, EVAS_CALLBACK_FREE, _mp_lyric_view_layout_del_cb,
					playing_view);

	/* Get window size */
	elm_win_screen_size_get(ad->win_main, NULL, NULL, &playing_view->lyric_view->win_w, &playing_view->lyric_view->win_h);
	endfunc;
}

void
mp_lyric_view_refresh(void *data)
{
	MpPlayerView_t *playing_view = (MpPlayerView_t *)data;
	MP_CHECK(playing_view);
	if (!playing_view->lyric_view) return;

	struct appdata *ad = mp_util_get_appdata();
	MP_CHECK(ad);


	if (playing_view->lyric_view->b_drag)
		return;
	...
	if (ad->lyric_mgr != NULL) {
		...
		for (index = 0; index < count-1; index++) {
			...
			if ((pos >= node1->time) && (pos < node2->time)) {
				_mp_lyric_view_set_current_line(playing_view, index);
				break;
			}
		}
	}
...
mp_lyric_view_hide(void *data)
{
	...
	struct appdata *ad = mp_util_get_appdata();
	MP_CHECK(ad);
	edje_object_signal_emit(_EDJ(playing_view->player_view_layout), "lrc_invisible", "lrc");

	endfunc;
}

void
mp_lyric_view_show(void *data)
{
	startfunc;
	...
	if (playing_view->lyric_view == NULL)
		mp_lyric_view_create(playing_view);
	...
	if (playing_view->lyric_view->music_path == NULL ||
		((playing_view->lyric_view->lyric_buffer == NULL) && (playing_view->lyric_view->synclrc_list == NULL))) {
		mp_lyric_view_create(playing_view);
	} else if (g_strcmp0(playing_view->lyric_view->music_path, current_item->uri)) {
		mp_lyric_view_create(playing_view);
	}
#else
	...
	if (playing_view->lyric_view->lyric_path != NULL) {
		if (strcmp(lyric_path, playing_view->lyric_view->lyric_path)) {
			mp_lyric_view_create(playing_view);
		}
	}
	IF_FREE(lyric_path);
	...
	edje_object_signal_emit(_EDJ(playing_view->player_view_layout), "lrc_visible", "lrc");
	endfunc;
}

void
mp_lyric_view_destroy(void *data)
{
	startfunc;
...
#ifdef MP_FEATURE_SUPPORT_ID3_TAG
	if (playing_view->lyric_view->lyric_buffer != NULL) {
		free(playing_view->lyric_view->lyric_buffer);
	}
#else
	if (playing_view->lyric_view->lyric_path != NULL) {
		free(playing_view->lyric_view->lyric_path);
	}
#endif
	if (playing_view->lyric_view->music_path != NULL) {
		free(playing_view->lyric_view->music_path);
	}

	IF_FREE(playing_view->lyric_view);

	endfunc;
}

void
mp_lyric_view_create(void *data)
{
	startfunc;
	...
	/* No lyric */
	if (lrc_data == NULL) {
		edje_object_signal_emit(_EDJ(playing_view->player_view_layout), "lrc_destroy", "lrc");
		return;
	}
#else
	...
	/* Init view data */
	_mp_lyric_view_init_data(playing_view);
	MP_CHECK(playing_view->lyric_view);

	/* Parse lyric file */
#ifdef MP_FEATURE_SUPPORT_ID3_TAG
	if (sync_num > 0) {
		playing_view->lyric_view->synclrc_list = lrc_data;
		mp_lyric_mgr_create(ad, lrc_data, MP_LYRIC_SOURCE_LIST);
	} else {
		playing_view->lyric_view->lyric_buffer = lrc_data;
		mp_lyric_mgr_create(ad, lrc_data, MP_LYRIC_SOURCE_BUFFER);
	}
#else
	playing_view->lyric_view->lyric_path = lyricPath;
	mp_lyric_mgr_create(ad, lyricPath, MP_LYRIC_SOURCE_FILE);
#endif
	SAFE_FREE(playing_view->lyric_view->music_path);
	playing_view->lyric_view->music_path = strdup(current_item->uri);

	/* Load lyric view */
	_mp_lyric_view_load_lyric(playing_view);
